...
import java.nio.channels.{ SocketChannel => JSocketChannel, SelectionKey => JSelectionKey }

import scalaz.nio.channels.{ Selector, ServerSocketChannel, SocketChannel }
import scalaz.zio.duration._
import scalaz.zio.{ Chunk, IO, RTS, Schedule }
    ...
    section(test("read/write") { () =>
                  ...
                  IO.when(key.isAcceptable) {
                    for {
                      ...
                      client <- IO.fromEither(
                                 clientOpt.toRight(new RuntimeException("option empty"))
                               )
      ...
      def client: IO[Exception, String] =
        for {
          ...
          bytes   = Chunk.fromArray("Hello".getBytes)
