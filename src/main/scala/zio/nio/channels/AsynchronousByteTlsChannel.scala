...
object AsynchronousTlsByteChannel  {
   ...
   def apply( raw_ch: AsynchronousSocketChannel, sslContext : SSLContext ) : ZIO[ Blocking with Clock, Exception, AsynchronousTlsByteChannel ] =
   {
       val R = for {
   ...
   private[nio] def open( raw_ch: AsynchronousByteChannel, ssl_engine : SSLEngine )  = 
   {
      ...
      val result = for {
        ...
        _ <- loop.repeat( zio.Schedule.doWhile( c => {  /*println( "do=" + c.toString );*/ c != FINISHED } )).refineToOrDie[Exception]
...
class AsynchronousTlsByteChannel( private val channel: AsynchronousSocketChannel, 
     ...
     def read( expected_size : Int ) : ZIO[Any with Clock, Exception, Chunk[Byte] ] =
     {
        val result = for {
           ...
           //p <- in.position
           //l <- in.limit
           //_ <- IO.effect( println( "raw read> pos=" + p + " " + "lim=" + l ) )
           ...
           loop = for { 
              ...
              _ <- if ( stat != SSLEngineResult.Status.OK  ) IO.fail( new Exception( "AsynchronousTlsByteChannel#read() " + res.toString()) ) else IO.unit
             rem <- in.remaining
           } yield( rem )  
           ...
           // _ <- IO.effect( println( "read> pos=" + pos + " " + "lim=" + limit + "sz=" + array.length ) )
   ...
   final def write(chunk: Chunk[Byte]): ZIO[Any with Clock, Exception, Int ] =
   {
     val res =  for {
         ...
         //_ <- IO.effect( println( chunk.length ) )
         ...
         loop        = for { 
                        ...
                        stat  <- IO.effect( res.getStatus() )
                        _ <- if ( stat != SSLEngineResult.Status.OK  ) IO.fail( new Exception( "AsynchronousTlsByteChannel#write() " + res.toString()) ) 
                             else IO.unit
                        rem   <- in.remaining 
                      } yield( rem ) 
         _           <- loop.repeat( zio.Schedule.doWhile( _ != 0 ) )          
         ...
         //pos         <- out.position
         //lim         <- out.limit
         //_ <- IO.effect( println( "write>  pos=" + pos + " " + "lim=" + lim ) )
...
object AsynchronousServerTlsByteChannel  {
   ...
   private[nio] def open( raw_ch: AsynchronousByteChannel, ssl_engine : SSLEngine )  = 
   {
      ...
      val result = for {
        ...
        r <- loop.repeat( zio.Schedule.doWhile( c => {  /*println( "do=" + c.toString );*/ c != FINISHED } )).refineToOrDie[Exception]
