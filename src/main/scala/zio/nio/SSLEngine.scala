...
class SSLEngine( val engine :JSSLEngine )  {
    ...
    {
      IO.effect( engine.wrap( src.buffer.asInstanceOf[JByteBuffer], dst.buffer.asInstanceOf[JByteBuffer] )).refineToOrDie[Exception] 
    }

    final def unwrap( src : Buffer[Byte], dst : Buffer[Byte] ) : IO[Exception, SSLEngineResult ] =
    {
      IO.effect( engine.unwrap( src.buffer.asInstanceOf[JByteBuffer], dst.buffer.asInstanceOf[JByteBuffer] )).refineToOrDie[Exception] 
    }
