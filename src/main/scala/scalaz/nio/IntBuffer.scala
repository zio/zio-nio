...
private[nio] class IntBuffer(val intBuffer: JIntBuffer) extends Buffer[Int](intBuffer) {
  ...
  override def array: IO[Exception, Array[Int]] = IO.effect(intBuffer.array()).refineOrDie(JustExceptions)

  def order: IO[Nothing, ByteOrder] = IO.succeed(intBuffer.order())

  def slice: IO[Exception, IntBuffer] = IO.effect(intBuffer.slice()).map(new IntBuffer(_)).refineOrDie(JustExceptions)

  override def get: IO[Exception, Int] = IO.effect(intBuffer.get()).refineOrDie(JustExceptions)

  override def get(i: Int): IO[Exception, Int] = IO.effect(intBuffer.get(i)).refineOrDie(JustExceptions)
