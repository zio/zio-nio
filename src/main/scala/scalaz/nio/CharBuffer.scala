...
private[nio] class CharBuffer(val charBuffer: JCharBuffer) extends Buffer[Char](charBuffer) {
  ...
  override def array: IO[Exception, Array[Char]] = IO.effect(charBuffer.array()).refineOrDie(JustExceptions)

  def order: IO[Nothing, ByteOrder] = IO.succeed(charBuffer.order())

  def slice: IO[Exception, CharBuffer] = IO.effect(charBuffer.slice()).map(new CharBuffer(_)).refineOrDie(JustExceptions)

  override def get: IO[Exception, Char] = IO.effect(charBuffer.get()).refineOrDie(JustExceptions)

  override def get(i: Int): IO[Exception, Char] = IO.effect(charBuffer.get(i)).refineOrDie(JustExceptions)
