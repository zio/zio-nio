 ...
 * [[https://docs.oracle.com/javase/8/docs/api/java/nio/Buffer.html the Java documentation]]
 * is applicable to this class.
 *
 * Buffer instances are in no way synchronized and are typically used from
 * a single fiber. Extract immutable `Chunk`s to pass values to other fibers
 * or via streams.
 *
 * =Construction=
 *
 * There is a concrete buffer subclass for each primitive type.
 * There are three ways to create a buffer:
 *
 * ==Allocation==
 *
 * Simply allocates a new buffer on the heap of a certain size,
 * for example `Buffer.byte(100)`, `Buffer.char(200)`.
 *
 * For `ByteBuffer` there is the special case of direct buffers, which
 * can be constructed via `Buffer.byteDirect`. See the Java docs for
 * details of the advantages and disadvantages of direct byte buffers.
 *
 * ==Wrapping==
 *
 * Buffers can wrap existing Java buffers, or ZIO `Chunk`s.
 * Care must be taken when wrapping Java buffers that
 * the wrapped buffer is not subsequently modified; these objects are
 * typically wrapped to provide efficient interoperability with Java APIs.
 *
 * A `CharBuffer` can also wrap any `java.lang.CharSequence`.
 *
 * ==Views==
 *
 * While `ByteBuffer`s are created via allocation or
 * wrapping, the other buffer types are more commonly constructed as a
 * ''view'' over a `ByteBuffer`.
 * Each numeric buffer class supports either big-endian or little-endian
 * byte order as required.
 *
 * View buffers are constructed via the various `asXXX` methods on the
 * [[zio.nio.core.ByteBuffer]] class, for example:
 *
 * {{{
 *   val ints: UIO[IntBuffer] = bytes.asIntBuffer
 * }}}
 *
 * Changes to made via view buffers are reflected in the original `ByteBuffer`
 * and vice-versa.
 *
 * =Differences from Java=
 *
 * The Java API supports "invocation chaining", which does not work with effect
 * values. The Java example:
 *
 * {{{
...
abstract class Buffer[A: ClassTag] private[nio] (buffer: JBuffer) {
   ...
   */
  def order: UIO[ByteOrder]

  /**
   * Returns this buffer's position.
   */
  final def position: UIO[Int] = UIO.effectTotal(buffer.position)
   ...
   *
   * @param delta The number of elements to move, negative to move backwards.
   ...
   */
  final def limit: UIO[Int] = UIO.effectTotal(buffer.limit)
   ...
   */
  final def remaining: UIO[Int] = UIO.effectTotal(buffer.remaining)

  /**
   * Indicates whether there are any elements between this buffer's
   * position and its limit.
   */
  final def hasRemaining: UIO[Boolean] = UIO.effectTotal(buffer.hasRemaining)

  /**
   * Sets this buffer's mark to the current position.
   */
  final def mark: UIO[Unit] = UIO.effectTotal(buffer.mark()).unit

  /**
   * Resets the position to the previously set mark.
   * A mark ''must'' be set before calling this.
   *
   * Dies with `InvalidMarkException` if a mark has not previously been set.
   */
  final def reset: UIO[Unit] = UIO.effectTotal(buffer.reset()).unit

  /**
   * Clears this buffer.
   * The position is set to zero, the limit is set to the capacity, and the mark is discarded.
   * No values in the buffer are actually cleared, but this is typically used
   * before putting new values into a buffer, after all its contents have
   * been processed.
   *
   * If the buffer's current values have not been completely processed, then
   * the `compact` method may be more appropriate.
   */
  final def clear: UIO[Unit] = UIO.effectTotal(buffer.clear()).unit
   ...
   */
  final def flip: UIO[Unit] = UIO.effectTotal(buffer.flip()).unit
   ...
   */
  final def rewind: UIO[Unit] = UIO.effectTotal(buffer.rewind()).unit
   ...
   */
  def slice: UIO[Buffer[A]]

  /**
   * Compacts this buffer  (optional operation).
   * The bytes between the buffer's current position and its limit, if any, are copied
   * to the beginning of the buffer.
   * That is, the byte at index `p = position()` is copied to index `0`,
   * the byte at index `p + 1` is copied to index `1`, and so forth until
   * the byte at index `limit() - 1` is copied to index `n = limit() - 1 - p`.
   ...
   */
  def duplicate: UIO[Buffer[A]]

  /**
   * Perform effects using this buffer's underlying array directly.
   * Because only some buffers are backed by arrays, two cases must be
   * handled. Ideally, the same result is produced in each case, with
   * the `hasArray` variant just being more efficient.
   *
   * For the `hasArray` case, the function is provided the backing array itself
   * and an offset within that array which contains the first element of this
   * buffer. Elements in the array before the offset are not contained in this
   * buffer.
   *
   * @param noArray The effect to perform if this buffer is not backed by an array.
   * @param hasArray The effect to perform if this buffer is backed by an array.
   */
  final def withArray[R, E, B](
    ...
    if (buffer.hasArray)
      for {
        a      <- array
        offset <- UIO.effectTotal(buffer.arrayOffset())
        result <- hasArray(a, offset)
      } yield result
    else
      noArray

  protected[nio] def array: UIO[Array[A]]

  /**
   * Relative get of a single element.
   * Reads the element at the position and increments the position.
   *
   ...
   * Reads the element at the given index. The position does not change.
   *
   ...
   *
   * Reads up to the specified number of elements from the current position.
   * If fewer than `maxLength` elements are remaining, then all the remaining
   * elements are read. The position is incremented by the number of elements
   * read.
   *
   * @param maxLength Defaults to `Int.MaxValue`, meaning all remaining
   *                  elements will be read.
   */
  def getChunk(maxLength: Int = Int.MaxValue): UIO[Chunk[A]]

  /**
   * Relative put of a single element.
   * Writes the element at the position and increments the position.
   *
   * Dies with `BufferOverflowException` if there are no elements remaining.
   * Dies with `ReadOnlyBufferException` if this is a read-only buffer.
   */
  def put(element: A): UIO[Unit]

  /**
   * Absolute put of a single element.
   * Writes the element at the specified index. The position does not change.
   *
   * Dies with `IndexOutOfBoundsException` if the index is negative or not smaller than the limit.
   * Dies with `ReadOnlyBufferException` if this is a read-only buffer.
   */
  def put(index: Int, element: A): UIO[Unit]
   ...
   */
  protected def putChunkAll(chunk: Chunk[A]): UIO[Unit]

  /**
   * Relative put of multiple elements.
   * Writes as many elements as can fit in remaining buffer space, returning
   * any elements that did not fit.
   *
   ...
   */
  def asReadOnlyBuffer: UIO[Buffer[A]]
...
object Buffer {
   ...
   *
   * The new buffer will have a capacity equal to the chunk length, its
   * position will be 0 and its limit set to the capacity.
   *
   ...
   *
   * This is only useful for inter-operating with Java APIs that provide
   * Java byte buffers.
   */
   ...
   *
   * The new buffer's capacity will be `charSequence.length`, its
   * position will be `start` and its limit will be `end`.
   *
   * Dies with `IndexOutOfBoundsException` if `start` or `end` are out of bounds.
   *
   * @param charSequence The characters to wrap.
   * @param start must be >= 0 and <= capacity
   * @param end must be >= start and <= capacity
   */
  def char(
   ...
   * Creates a read-only character buffer wrapping a character sequence.
   *
   * The new buffer's capacity and limit will be `charSequence.length` and its
   * position will be 0.
   *
   * @param charSequence The characters to wrap.
   */
  def char(charSequence: CharSequence): UIO[CharBuffer] =
    UIO.effectTotal(new CharBuffer(JCharBuffer.wrap(charSequence)))
   ...
   * @param chunk The data to copy into the new buffer.
   */
  def double(chunk: Chunk[Double]): UIO[DoubleBuffer] =
    UIO.effectTotal(doubleFromJava(JDoubleBuffer.wrap(chunk.toArray)))
