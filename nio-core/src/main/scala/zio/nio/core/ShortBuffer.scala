...
final class ShortBuffer(shortBuffer: JShortBuffer) extends Buffer[Short](shortBuffer) {
  ...
  override protected[nio] def array: UIO[Array[Short]] =
    UIO.effectTotal(shortBuffer.array())

  override def order: UIO[ByteOrder] = UIO.effectTotal(shortBuffer.order())

  override def slice: UIO[ShortBuffer] =
    UIO.effectTotal(new ShortBuffer(shortBuffer.slice()))

  override def compact: UIO[Unit] =
    UIO.effectTotal(shortBuffer.compact()).unit

  override def duplicate: UIO[ShortBuffer] =
    UIO.effectTotal(new ShortBuffer(shortBuffer.duplicate()))
  ...
  override def get: UIO[Short] =
    UIO.effectTotal(shortBuffer.get())

  override def get(i: Int): UIO[Short] =
    UIO.effectTotal(shortBuffer.get(i))
  ...
  override def put(element: Short): UIO[Unit] =
    UIO.effectTotal(shortBuffer.put(element)).unit

  override def put(index: Int, element: Short): UIO[Unit] =
    UIO.effectTotal(shortBuffer.put(index, element)).unit
  ...
  override def asReadOnlyBuffer: UIO[ShortBuffer] =
    UIO.effectTotal(new ShortBuffer(shortBuffer.asReadOnlyBuffer()))
