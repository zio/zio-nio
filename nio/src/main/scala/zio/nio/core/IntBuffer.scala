...
final class IntBuffer(intBuffer: JIntBuffer) extends Buffer[Int](intBuffer) {
  ...
  override protected[nio] def array: UIO[Array[Int]] =
    UIO.effectTotal(intBuffer.array())

  override def order: UIO[ByteOrder] = UIO.effectTotal(intBuffer.order)

  override def slice: UIO[IntBuffer] =
    UIO.effectTotal(new IntBuffer(intBuffer.slice()))

  override def compact: UIO[Unit] =
    UIO.effectTotal(intBuffer.compact()).unit

  override def duplicate: UIO[IntBuffer] =
    UIO.effectTotal(new IntBuffer(intBuffer.duplicate()))
  ...
  override def get: UIO[Int] =
    UIO.effectTotal(intBuffer.get())

  override def get(i: Int): UIO[Int] =
    UIO.effectTotal(intBuffer.get(i))
  ...
  override def put(element: Int): UIO[Unit] =
    UIO.effectTotal(intBuffer.put(element)).unit

  override def put(index: Int, element: Int): UIO[Unit] =
    UIO.effectTotal(intBuffer.put(index, element)).unit
  ...
  override def asReadOnlyBuffer: UIO[IntBuffer] =
    UIO.effectTotal(new IntBuffer(intBuffer.asReadOnlyBuffer()))
