...
final class IntBuffer(protected[nio] val buffer: JIntBuffer) extends Buffer[Int] {
  ...
  override protected[nio] def array: UIO[Array[Int]] = UIO.succeed(buffer.array())

  override def order: UIO[ByteOrder] = UIO.succeed(buffer.order)

  override def slice: UIO[IntBuffer] = UIO.succeed(new IntBuffer(buffer.slice()))

  override def compact: UIO[Unit] = UIO.succeed(buffer.compact()).unit

  override def duplicate: UIO[IntBuffer] = UIO.succeed(new IntBuffer(buffer.duplicate()))
  ...
  override def get: UIO[Int] = UIO.succeed(buffer.get())

  override def get(i: Int): UIO[Int] = UIO.succeed(buffer.get(i))
  ...
  override def put(element: Int): UIO[Unit] = UIO.succeed(buffer.put(element)).unit

  override def put(index: Int, element: Int): UIO[Unit] = UIO.succeed(buffer.put(index, element)).unit
  ...
  override def asReadOnlyBuffer: UIO[IntBuffer] = UIO.succeed(new IntBuffer(buffer.asReadOnlyBuffer()))
