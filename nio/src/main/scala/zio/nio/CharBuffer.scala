...
final class CharBuffer(protected[nio] val buffer: JCharBuffer) extends Buffer[Char] {
  ...
  override protected[nio] def array: UIO[Array[Char]] = UIO.succeed(buffer.array())

  override def order: UIO[ByteOrder] = UIO.succeed(buffer.order())

  override def slice: UIO[CharBuffer] = UIO.succeed(new CharBuffer(buffer.slice()))

  override def compact: UIO[Unit] = UIO.succeed(buffer.compact()).unit

  override def duplicate: UIO[CharBuffer] = UIO.succeed(new CharBuffer(buffer.duplicate()))
  ...
  override def get: UIO[Char] = UIO.succeed(buffer.get())

  override def get(i: Int): UIO[Char] = UIO.succeed(buffer.get(i))
  ...
  def getString: UIO[String] = UIO.succeed(buffer.toString())

  override def put(element: Char): UIO[Unit] = UIO.succeed(buffer.put(element)).unit

  override def put(index: Int, element: Char): UIO[Unit] = UIO.succeed(buffer.put(index, element)).unit
  ...
  override def asReadOnlyBuffer: UIO[CharBuffer] = UIO.succeed(new CharBuffer(buffer.asReadOnlyBuffer()))
