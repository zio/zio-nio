...
class ByteBuffer protected[nio] (protected[nio] val buffer: JByteBuffer) extends Buffer[Byte] {
  ...
  final override protected[nio] def array: UIO[Array[Byte]] = UIO.succeed(buffer.array())

  final def order: UIO[ByteOrder] = UIO.succeed(buffer.order())

  /**
   * Changes the byte order used by this buffer for multi-byte reads and view buffers.
   */
  def order(o: ByteOrder): UIO[Unit] = UIO.succeed(buffer.order(o)).unit

  final override def slice: UIO[ByteBuffer] = UIO.succeed(new ByteBuffer(buffer.slice()))

  final override def compact: UIO[Unit] = UIO.succeed(buffer.compact()).unit

  final override def duplicate: UIO[ByteBuffer] = UIO.succeed(new ByteBuffer(buffer.duplicate()))
  ...
  final override def get: UIO[Byte] = UIO.succeed(buffer.get())

  final override def get(i: Int): UIO[Byte] = UIO.succeed(buffer.get(i))
  ...
  final override def put(element: Byte): UIO[Unit] = UIO.succeed(buffer.put(element)).unit

  final override def put(index: Int, element: Byte): UIO[Unit] = UIO.succeed(buffer.put(index, element)).unit

  def putByteBuffer(source: ByteBuffer): UIO[Unit] = UIO.succeed(buffer.put(source.buffer)).unit
  ...
  final override def asReadOnlyBuffer: UIO[ByteBuffer] = UIO.succeed(new ByteBuffer(buffer.asReadOnlyBuffer()))

  final def asCharBuffer: UIO[CharBuffer] = UIO.succeed(new CharBuffer(buffer.asCharBuffer()))

  final def asDoubleBuffer: UIO[DoubleBuffer] = UIO.succeed(new DoubleBuffer(buffer.asDoubleBuffer()))

  final def asFloatBuffer: UIO[FloatBuffer] = UIO.succeed(new FloatBuffer(buffer.asFloatBuffer()))

  final def asIntBuffer: UIO[IntBuffer] = UIO.succeed(new IntBuffer(buffer.asIntBuffer()))

  final def asLongBuffer: UIO[LongBuffer] = UIO.succeed(new LongBuffer(buffer.asLongBuffer()))

  final def asShortBuffer: UIO[ShortBuffer] = UIO.succeed(new ShortBuffer(buffer.asShortBuffer()))
   ...
   */
  final def putChar(value: Char): UIO[Unit] = UIO.succeed(buffer.putChar(value)).unit
   ...
   */
  final def putChar(index: Int, value: Char): UIO[Unit] = UIO.succeed(buffer.putChar(index, value)).unit
   ...
   * if this is a read-only buffer.
   */
  final def putDouble(value: Double): UIO[Unit] = UIO.succeed(buffer.putDouble(value)).unit
   ...
   * `ReadOnlyBufferException` if this is a read-only buffer.
   */
  final def putDouble(index: Int, value: Double): UIO[Unit] = UIO.succeed(buffer.putDouble(index, value)).unit
   ...
   * if this is a read-only buffer.
   */
  final def putFloat(value: Float): UIO[Unit] = UIO.succeed(buffer.putFloat(value)).unit
   ...
   * `ReadOnlyBufferException` if this is a read-only buffer.
   */
  final def putFloat(index: Int, value: Float): UIO[Unit] = UIO.succeed(buffer.putFloat(index, value)).unit
   ...
   * if this is a read-only buffer.
   */
  final def putInt(value: Int): UIO[Unit] = UIO.succeed(buffer.putInt(value)).unit
   ...
   * `ReadOnlyBufferException` if this is a read-only buffer.
   */
  final def putInt(index: Int, value: Int): UIO[Unit] = UIO.succeed(buffer.putInt(index, value)).unit
   ...
   * if this is a read-only buffer.
   */
  final def putLong(value: Long): UIO[Unit] = UIO.succeed(buffer.putLong(value)).unit
   ...
   * `ReadOnlyBufferException` if this is a read-only buffer.
   */
  final def putLong(index: Int, value: Long): UIO[Unit] = UIO.succeed(buffer.putLong(index, value)).unit
   ...
   * if this is a read-only buffer.
   */
  final def putShort(value: Short): UIO[Unit] = UIO.succeed(buffer.putShort(value)).unit
   ...
   * `ReadOnlyBufferException` if this is a read-only buffer.
   */
  final def putShort(index: Int, value: Short): UIO[Unit] = UIO.succeed(buffer.putShort(index, value)).unit
   ...
   */
  final def getChar: UIO[Char] = UIO.succeed(buffer.getChar())
   ...
   */
  final def getChar(index: Int): UIO[Char] = UIO.succeed(buffer.getChar(index))
   ...
   */
  final def getDouble: UIO[Double] = UIO.succeed(buffer.getDouble())
   ...
   */
  final def getDouble(index: Int): UIO[Double] = UIO.succeed(buffer.getDouble(index))
   ...
   */
  final def getFloat: UIO[Float] = UIO.succeed(buffer.getFloat())
   ...
   */
  final def getFloat(index: Int): UIO[Float] = UIO.succeed(buffer.getFloat(index))
   ...
   * Dies with `BufferUnderflowException` If there are fewer than 4 bytes remaining.
   */
  final def getInt: UIO[Int] = UIO.succeed(buffer.getInt())
   ...
   * Dies with `IndexOutOfBoundsException` if the index is negative or not smaller than the limit-3.
   */
  final def getInt(index: Int): UIO[Int] = UIO.succeed(buffer.getInt(index))
   ...
   * Dies with `BufferUnderflowException` If there are fewer than 8 bytes remaining.
   */
  final def getLong: UIO[Long] = UIO.succeed(buffer.getLong())
   ...
   * Dies with `IndexOutOfBoundsException` if the index is negative or not smaller than the limit-7.
   */
  final def getLong(index: Int): UIO[Long] = UIO.succeed(buffer.getLong(index))
   ...
   * Dies with `BufferUnderflowException` If there are fewer than 2 bytes remaining.
   */
  final def getShort: UIO[Short] = UIO.succeed(buffer.getShort())
   ...
   * Dies with `IndexOutOfBoundsException` if the index is negative or not smaller than the limit-1.
   */
  final def getShort(index: Int): UIO[Short] = UIO.succeed(buffer.getShort(index))
